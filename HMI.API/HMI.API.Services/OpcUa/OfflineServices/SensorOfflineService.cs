...
namespace HMI.API.Services.OpcUa.OfflineServices
{
    ...
    public class SensorOfflineService : ISensorService
    {
        ...
        {
            this.logger.LogDebug($"Get sensors for module with id {moduleId}");
            return this.sensorList.Where(s => s.ModuleId == moduleId);
        }

        /// <inheritdoc/>
        public SensorResponse GetById(int sensorId)
        {
            return this.sensorList.FirstOrDefault(s => s.Id == sensorId);
        }

        /// <inheritdoc/>
        public SensorResponse Add(int moduleId, string sensorName)
        {
            var nextId = this.sensorList.Count + 1;
            ...
            this.configList.Add(new ConfigResponse { ModuleId = moduleId, SensorId = nextId, SensorName = sensorName });
            this.extrinsicCalibList.Add(new ExtrinsicCalibResponse { ModuleId = moduleId, SensorId = nextId, });
            this.referenceList.Add(new ReferenceResponse { ModuleId = moduleId, SensorId = nextId, });
            return newSensor;
        }
        ...
        {
            var sensor = this.sensorList.FirstOrDefault(s => s.Id == id);
            if (sensor == null)
            {
            ...
            this.configList.Remove(this.configList.FirstOrDefault(s => s.SensorId == id));
            this.extrinsicCalibList.Remove(this.extrinsicCalibList.FirstOrDefault(s => s.SensorId == id));
            this.referenceList.Remove(this.referenceList.FirstOrDefault(s => s.SensorId == id));
        }
        ...
        {
            return this.GetExtrinsicCalib(id);
        }

        /// <inheritdoc/>
        public void ExtrinsicCalibration(int id, UpdateExtrCalibRequest request)
        {
            var extrinsicCalib = this.GetExtrinsicCalib(id);
        ...
        {
            var reference = this.referenceList.FirstOrDefault(sc => sc.SensorId == id);
            if (reference == null)
            {
                throw new KeyNotFoundException($"Could not find the sensor with id {id}");
            }
        ...
        {
            var config = this.configList.FirstOrDefault(sc => sc.SensorId == id);
            if (config == null)
            {
                throw new KeyNotFoundException($"Could not find the sensor with id {id}");
            }
        ...
        {
            var extrinsicCalib = this.extrinsicCalibList.FirstOrDefault(sc => sc.SensorId == id);
        ...
        {
            this.sensorList = new List<SensorResponse>
                                  {
            ...
            this.configList = new List<ConfigResponse>();
            this.extrinsicCalibList = new List<ExtrinsicCalibResponse>();
            this.referenceList = new List<ReferenceResponse>();

            var rnd = new Random();

            foreach (var sensor in this.sensorList)
            {
                ...
                this.configList.Add(configSensor);
                ...
                this.extrinsicCalibList.Add(extrinsicCalib);
