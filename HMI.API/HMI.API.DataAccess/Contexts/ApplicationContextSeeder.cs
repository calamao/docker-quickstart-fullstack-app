...
namespace HMI.API.DataAccess.Contexts
{
    using BCrypt.Net;
    ...
    public class ApplicationContextSeeder
    {
        ...
        public async Task SeedAsync(ApplicationContext context)
        {
            ...
            {
                this.logger.LogInformation("Initialize DB Seed");

                await this.SeedItemsAsync(context);
                await this.SeedUsersAsync(context);
                await this.SeedLanguagesAsync(context);
            });
        }
        ...
        private async Task SeedItemsAsync(ApplicationContext context)
        {
            ...
            {
                this.logger.LogDebug("Seeding Items");
        ...
        private async Task SeedUsersAsync(ApplicationContext context)
        {
            ...
            {
                this.logger.LogDebug("Seeding preconfigured Users");
                var preconfiguredRoles = new List<User>()
                {
                    new User { Username = "oper", PasswordHash = BCrypt.HashPassword("Test123."), FirstName = "Operator", Role = Role.Operator, Created = DateTime.UtcNow, IsDefault = true },
                    new User { Username = "main", PasswordHash = BCrypt.HashPassword("Test123."), FirstName = "Maintenance", Role = Role.Maintenance, Created = DateTime.UtcNow, IsDefault = true },
                    new User { Username = "comm", PasswordHash = BCrypt.HashPassword("Test123."), FirstName = "Commissioning", Role = Role.Commissioning, Created = DateTime.UtcNow, IsDefault = true },
                    new User { Username = "admin", PasswordHash = BCrypt.HashPassword("Test123."), FirstName = "Administrator", Role = Role.Admin, Created = DateTime.UtcNow, IsDefault = true },
                };
        ...
        private async Task SeedLanguagesAsync(ApplicationContext context)
        {
            ...
            {
                this.logger.LogDebug("Seeding preconfigured Languages");
