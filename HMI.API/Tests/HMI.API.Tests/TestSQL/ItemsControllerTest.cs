...
namespace HMI.API.Tests.TestSQL
{
    public abstract class ItemsControllerTest
    {
        ...
        #region Utils
        private ItemsController GetTarget(ApplicationContext context)
        {
            return new ItemsController(new ItemsService(context));
        }
        #endregion
        ...
        public void Can_get_items()
        {
            using (var context = new ApplicationContext(ContextOptions))
            {
                var controller = GetTarget(context);
